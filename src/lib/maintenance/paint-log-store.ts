...
export async function createPaintLogEntry(input: PaintLogInput): Promise<{ ok: boolean; error?: string }> {
  const id = nextId();
  const result = await store.create({ id, ...input });
  return result !== null ? { ok: true } : { ok: false, error: store.getLastError() ?? undefined };
}
export async function updatePaintLogEntry(id: string, input: PaintLogInput): Promise<{ ok: boolean; error?: string }> {
