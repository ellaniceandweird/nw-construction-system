...
/**
 * Watches Budgets/Estimates/Change Orders/Purchase Orders for anything
 * sitting in a pending-approval-like status, and auto-creates a tracking
 * row in approval_requests for any that don't have one yet — so nothing
 * that needs sign-off can silently fall outside the Approvals module.
...
export function useAutoConsolidateApprovals(
  ...
  React.useEffect(() => {
    ...
    // creating duplicates.
  }, [budgets, estimates, changeOrders, purchaseOrders, projects, existingRequests]);
}
