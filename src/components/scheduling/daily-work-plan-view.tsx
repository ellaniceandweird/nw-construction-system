...
import { getDetailsForDate } from "@/lib/scheduling/daily-work-plan-details-store";
...
export function DailyWorkPlanView() {
  ...
  function handleGenerateImage() {
    ...
    const details = getDetailsForDate(dateStr);
    const enriched = selectedScheduled.map((item) => {
      const assignment = details?.taskAssignments.find((t) => t.activityId === item.activity.id);
      const assignedCrewNames = assignment?.assignedEmployeeIds
        .map((id) => details?.crewAttendance.find((c) => c.employeeId === id)?.employeeName)
        .filter((n): n is string => !!n);
      return {
        ...item,
        assignedCrewNames: assignedCrewNames?.length ? assignedCrewNames : undefined,
        dailyCompletionTarget: assignment?.dailyCompletionTarget || undefined,
      };
    });
    generateDailyWorkPlanImage(date, enriched, projects);
  }
