...
import { createEquipmentMaintenanceBulk } from "@/lib/maintenance/equipment-maintenance-store";
import { parseEquipmentMaintenanceExcelFile } from "@/lib/maintenance/import/parse-equipment-maintenance-excel";
import { parseEquipmentMaintenancePdfFile } from "@/lib/maintenance/import/parse-equipment-maintenance-pdf";
import { showErrorToast } from "@/lib/toast/toast-store";
import type { ParsedEquipmentMaintenanceRow } from "@/lib/maintenance/import/parse-equipment-maintenance-excel";
...
export function ImportEquipmentMaintenanceDialog({ open, onOpenChange }: Props) {
  ...
  async function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const result = ext === "pdf" ? await parseEquipmentMaintenancePdfFile(file) : await parseEquipmentMaintenanceExcelFile(file);
      setRows(result.rows);
  ...
  function updateRow(index: number, patch: Partial<ParsedEquipmentMaintenanceRow>) {
  ...
  async function handleConfirm() {
    setSaving(true);
    const result = await createEquipmentMaintenanceBulk(
      rows.map((r) => ({
        propertyName: r.propertyName,
        location: r.location,
        systemType: r.systemType,
        maintenanceNeeded: r.maintenanceNeeded || undefined,
  ...
  const canConfirm = rows.length > 0 && rows.every((r) => r.systemType || r.location);
              ...
              {rows.map((r, i) => (
                <div key={i} className="grid grid-cols-1 gap-2 rounded-md border border-border p-2 sm:grid-cols-[1fr_1fr_auto]">
                  <div>
                    ...
                    <Input className="mt-1" value={r.systemType} onChange={(e) => updateRow(i, { systemType: e.target.value })} />
                  </div>
                  <div>
                    <Label className="text-xs">Location</Label>
                    <Input className="mt-1" value={r.location} onChange={(e) => updateRow(i, { location: e.target.value })} />
                  </div>
                  ...
                  <div>
                    ...
                    <Input className="mt-1" value={r.propertyName} onChange={(e) => updateRow(i, { propertyName: e.target.value })} />
                  </div>
                  <div>
                    <Label className="text-xs">Frequency</Label>
                    <Input className="mt-1" placeholder="e.g. 3-months" value={r.frequency ?? ""} onChange={(e) => updateRow(i, { frequency: e.target.value })} />
                  </div>
                  <div>
                    <Label className="text-xs">Last Completed</Label>
                    <Input type="date" className="mt-1" value={r.lastCompleted ?? ""} onChange={(e) => updateRow(i, { lastCompleted: e.target.value })} />
                  </div>
                  <div className="sm:col-span-2">
                    <Label className="text-xs">Maintenance Needed</Label>
                    <Input className="mt-1" value={r.maintenanceNeeded ?? ""} onChange={(e) => updateRow(i, { maintenanceNeeded: e.target.value })} />
                  </div>
                </div>
              ))}
              {rows.length === 0 && (
