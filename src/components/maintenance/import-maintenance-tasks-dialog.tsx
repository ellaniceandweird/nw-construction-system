...
import { addMaintenanceTasksBulk } from "@/lib/maintenance/maintenance-task-store";
import { parseMaintenanceTasksExcelFile } from "@/lib/maintenance/import/parse-maintenance-tasks-excel";
...
export function ImportMaintenanceTasksDialog({ open, onOpenChange }: Props) {
  ...
  async function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const result = ext === "pdf" ? await parseMaintenanceTasksPdfFile(file) : await parseMaintenanceTasksExcelFile(file);
      setRows(result.rows);
  ...
  function updateRow(index: number, patch: Partial<ParsedMaintenanceTaskRow>) {
  ...
  async function handleConfirm() {
    setSaving(true);
    const result = await addMaintenanceTasksBulk(
      rows.map((r) => ({
  ...
  const canConfirm = rows.length > 0 && rows.every((r) => r.taskDescription);
        ...
        {stage === "review" && (
          <div className="flex flex-col gap-4">
            ...
            <p className="text-sm text-muted-foreground">
              {rows.length} task{rows.length === 1 ? "" : "s"} found — edit anything below before saving.
            </p>
            <div className="flex max-h-96 flex-col gap-2 overflow-y-auto">
              {rows.map((r, i) => (
                <div key={i} className="grid grid-cols-1 gap-2 rounded-md border border-border p-2 sm:grid-cols-[1fr_1fr_auto]">
                  <div className="sm:col-span-2">
                    ...
                    <Input className="mt-1" value={r.taskDescription} onChange={(e) => updateRow(i, { taskDescription: e.target.value })} />
                  </div>
                  ...
                  <div>
                    ...
                    <Input className="mt-1" placeholder="low / medium / high" value={r.priority ?? ""} onChange={(e) => updateRow(i, { priority: e.target.value })} />
                  </div>
                  <div>
                    <Label className="text-xs">Responsible Party</Label>
                    <Input className="mt-1" value={r.responsibleParty ?? ""} onChange={(e) => updateRow(i, { responsibleParty: e.target.value })} />
                  </div>
                  <div>
              ...
              {rows.length === 0 && (
                <p className="rounded-md border border-dashed border-border p-4 text-center text-sm text-muted-foreground">
                  No tasks parsed from this file.
                </p>
              )}
            </div>
          </div>
          ...
          {stage === "review" && (
            <>
              ...
              <Button onClick={handleConfirm} disabled={!canConfirm || saving}>
                {saving ? "Saving…" : `Save ${rows.length} Task${rows.length === 1 ? "" : "s"}`}
              </Button>
            </>
          )}
