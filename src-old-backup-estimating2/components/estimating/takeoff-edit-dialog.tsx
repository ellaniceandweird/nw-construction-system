...
import { createTakeoffItem, updateTakeoffItem } from "@/lib/estimating/takeoff-store";
...
const UNIT_OPTIONS: MeasurementUnit[] = [
  "sf", "sq", "lf", "cy", "pieces", "each", "hours", "tons", "gallons", "pounds", "meters", "custom",
];
...
export function TakeoffEditDialog({ item, open, onOpenChange }: Props) {
  ...
  function handleSave() {
    ...
    if (item) {
      updateTakeoffItem(item.id, input);
    } else {
      createTakeoffItem(input);
    }
    onOpenChange(false);
  }
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              If set, Procurement&apos;s Forecast tab will use this quantity for matching
              schedule activities instead of its own illustrative default.
            </p>
          </div>
