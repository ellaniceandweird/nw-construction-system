...
interface Props {
  /** null = creating a new code, otherwise editing this one. */
...
export function CostCodeEditDialog({ costCode, open, onOpenChange }: Props) {
  ...
  function handleSave() {
    ...
    if (costCode) {
      updateCostCode(costCode.id, input);
    } else {
      createCostCode(input);
    }
    onOpenChange(false);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{costCode ? `Edit Cost Code ${costCode.code}` : "New Cost Code"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input id="code" className="mt-1.5" placeholder="e.g. 074600" value={code} onChange={(e) => setCode(e.target.value)} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                id="division"
                className="mt-1.5"
                placeholder="e.g. Division 07 — Thermal & Moisture Protection"
                value={division}
            ...
            <div>
              ...
              <Input id="category" className="mt-1.5" value={category} onChange={(e) => setCategory(e.target.value)} />
            </div>
          </div>
        </div>
