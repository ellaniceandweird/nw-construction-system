...
import { createEstimate, updateEstimate, withComputedLineItemTotals } from "@/lib/estimating/estimate-store";
...
interface Props {
  /** null = creating a new estimate, otherwise editing this one. */
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <Input
              id="taxMethod"
              className="mt-1.5"
              placeholder="e.g. Sales tax on materials"
              value={taxMethod}
            ...
            <div className="flex flex-col gap-3">
              {lineItems.map((li, index) => (
                <div key={index} className="rounded-md border border-border p-3">
                  <div className="grid grid-cols-1 items-end gap-2 sm:grid-cols-[1fr_140px_auto]">
                    <div>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-6">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className="mt-1"
                        value={li.laborCost}
                        onChange={(e) => updateLineItem(index, { laborCost: parseFloat(e.target.value) || 0 })}
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        className="mt-1"
                        value={li.equipmentCost}
                        onChange={(e) => updateLineItem(index, { equipmentCost: parseFloat(e.target.value) || 0 })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type="number"
                        className="mt-1"
                        value={li.subcontractCost}
                        onChange={(e) => updateLineItem(index, { subcontractCost: parseFloat(e.target.value) || 0 })}
                      />
                    </div>
                  </div>
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-2">
                    <div>
                      ...
                      <Input
                        type="number"
                        className="mt-1"
                        value={li.markupPercent ?? 0}
                        onChange={(e) => updateLineItem(index, { markupPercent: parseFloat(e.target.value) || 0 })}
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 rounded-md px-2 py-2 text-sm font-medium text-foreground">
                        {currency(computeLineItemTotal(li))}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {INDIRECT_FIELDS.map((f) => (
                <div key={f.key}>
                  ...
                  <Input
                    type="number"
                    className="mt-1"
                    value={indirectCosts[f.key] ?? ""}
                    onChange={(e) =>
                      setIndirectCosts((prev) => ({
                        ...prev,
                        [f.key]: e.target.value ? parseFloat(e.target.value) : undefined,
                      }))
                    }
                  />
                </div>
