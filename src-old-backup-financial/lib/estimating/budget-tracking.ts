...
import type { PurchaseOrder, ChangeOrder } from "@/types";
 ...
 * Approved change orders are folded into the "estimated" side of their
 * division, so the revised budget (not just the original bid) is what's
 * being compared against actual spend.
 */
export function computeDivisionBudget(
  ...
  for (const co of approvedChangeOrders) {
    const key = getDivisionSortKey(co.costCode);
    estimatedByDivision.set(key, (estimatedByDivision.get(key) ?? 0) + co.costImpact);
  }
