...
function PropertyPhotoThumbnail({ url, alt }: { url?: string; alt: string }) {
...
export function PropertyDetailDialog({ property, open, onOpenChange }: Props) {
  ...
  const propertyPhotos = getPropertyPhotos(relatedProjects, allPhotos);
  const billingEntity = billingEntities.find((b) => b.id === property.billingEntityId);
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              {propertyPhotos.length > 0 && (
                <Link href="/documents?tab=photos" className="group flex items-center gap-1 text-xs text-primary hover:underline">
                  View all in Documents <ArrowUpRight className="size-3" />
                </Link>
              )}
            </div>
            {propertyPhotos.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                No photos yet — add some from the{" "}
                <Link href="/documents?tab=photos" className="text-primary hover:underline">Photos tab in Documents</Link>.
              </p>
            ) : (
              <div className="grid grid-cols-4 gap-2 sm:grid-cols-6">
                {propertyPhotos.slice(0, 12).map((photo) => (
                  <a key={photo.id} href={photo.fileUrl} target="_blank" rel="noopener noreferrer" className="block">
                    <PropertyPhotoThumbnail url={photo.thumbnailUrl} alt={photo.caption ?? ""} />
                  </a>
                ))}
              </div>
            )}
