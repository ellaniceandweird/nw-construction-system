...
export function createCostCode(input: CostCodeInput) {
  ...
  const newCode: CostCode = { id: nextId(), createdBy: "user", createdDate: now, lastModifiedBy: "user", lastModifiedDate: now, revisionNumber: 1, module: "Estimating", status: "active", ...input };
...
export function updateCostCode(id: string, input: CostCodeInput) {
  costCodes = costCodes.map((c) => (c.id === id ? { ...c, ...input, lastModifiedDate: new Date().toISOString() } : c));
  persist();
  emit();
}
