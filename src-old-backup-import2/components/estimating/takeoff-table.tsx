  ...
  return (
    <>
      <div className="mb-3 flex items-center justify-between gap-3">
        <p className="text-xs text-muted-foreground">Quantity takeoffs by project. Items with a Forecast match feed Procurement&apos;s Forecast tab quantities automatically.</p>
        <Button size="sm" onClick={() => setCreating(true)} className="shrink-0"><Plus className="size-3.5" /> New Takeoff Item</Button>
      </div>
          ...
          <tbody>
            {sorted.map((t) => (
              <tr key={t.id} className="border-b border-border/60 last:border-0 hover:bg-accent/40">
                ...
                <td className="px-4 py-3 text-muted-foreground">{t.quantity.toLocaleString()} {t.unit}</td>
                <td className="px-4 py-3 text-muted-foreground">{t.wasteFactorPercent != null ? `${t.wasteFactorPercent}%` : "—"}</td>
                <td className="px-4 py-3 text-foreground">{t.adjustedQuantity.toLocaleString()} {t.unit}</td>
                <td className="px-4 py-3">
                  {t.materialKey ? (<Badge className="bg-info-soft text-info-foreground border-transparent">{t.materialKey}</Badge>) : (<span className="text-muted-foreground">—</span>)}
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" onClick={() => setEditing(t)}><Pencil className="size-3.5" /></Button>
                    <Button variant="ghost" size="icon" onClick={() => deleteTakeoffItem(t.id)}><Trash2 className="size-3.5 text-destructive" /></Button>
                  </div>
                </td>
              </tr>
            ))}
            {sorted.length === 0 && (<tr><td colSpan={8} className="px-4 py-6 text-center text-muted-foreground">No takeoff items yet — add one above.</td></tr>)}
          </tbody>
        </table>
      </Card>
